...
- **마지막 문서 업데이트**: 2026-02-27 (44차)

---

## 2026-02-27 (44차) 일반 상태 이석 체크·최초 로그인 시 설정 전체 조회

**작업 요약**

| 구분 | 내용 |
|------|------|
| **일반 상태 이석 체크** | `isAlreadyLockedByWorkHours()`를 **pcOnYmdTime/pcOffYmdTime** 기준으로 변경. 근무 시간대(pcOn ≤ now < pcOff)이면 false 반환해 이석 체크 적용. 기존 resolveScreenType(exCountRenewal)만 쓰면 일반 상태에서도 off/before로 잘못 판단되어 이석 미동작하던 문제 해소. |
| **최초 로그인 시 이석 미동작** | 앱을 로그인 창으로만 띄운 뒤 로그인하면 `leaveSeatDetector.start()`가 호출되지 않아 이석 감지가 아예 동작하지 않던 문제. **pcoff:checkLockAndShow** 핸들러에서 leaveSeatDetector.start()·updatePolicy() 호출 추가. |
| **로그인 직후 설정 전체 조회** | state.json에는 로그인 정보만 있으므로, 로그인 성공 시 **1회 근태·설정 전체 조회** 수행. invalidateRuntimeConfigCache 후 fetchWorkTimeWithLockScreen(getPcOffWorkTime + getLockPolicy/getLockScreenInfo)로 lastWorkTimeData·이석 정책·긴급해제 정책 반영. 로그 "근태·설정 전체 (로그인 직후 조회)"로 근태·이석·긴급·잠금화면 문구 등 필드 출력. offlineManager.reportApiSuccess() 호출. |
| **기타** | setTestModeEnabled에서 wins 필터 타입 가드 `(w): w is BrowserWindow` 적용해 TS18047 해소. |

**파일 변경**

- `app/main/index.ts`: isAlreadyLockedByWorkHours() pcOn/pcOff 기준, pcoff:checkLockAndShow에서 invalidateRuntimeConfigCache·fetchWorkTimeWithLockScreen·leaveSeatDetector.start/updatePolicy·로그 확장·reportApiSuccess, setTestModeEnabled filter 타입 가드
- `docs/개발_이력_리포트.md`: 44차 추가
- `docs/잠금_및_적용정책_설명.md`: state.json vs 설정값(API 조회) 구분 문단
- `docs/팀_공유_완료현황_및_남은일.md`: 이석·로그인 직후 조회 반영, 문서일·리포트 참조
