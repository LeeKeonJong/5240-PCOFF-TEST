...
| **이석 잠금·해제** | 이석 감지 시 설정된 이석 잠금화면(config.lockScreen.leave) 적용·주/보조 동일 데이터 전달. 이석 화면에서 PC-ON 클릭 시 비밀번호 모달(leaveSeatUnlockRequirePassword)·검증 동작. 보조 모니터 잠금 간혹 미표시 시 500ms 재시도로 보완. **일반 상태에서도 이석 체크** 적용(isAlreadyLockedByWorkHours를 pcOn/pcOff 기준으로 변경). **최초 로그인 시** leaveSeatDetector 시작·로그인 직후 1회 근태·설정 전체 조회(fetchWorkTimeWithLockScreen)로 이석·긴급해제 등 각종 설정 반영. |
...
| `docs/개발_이력_리포트.md` | 차수별 변경 이력(44차: 일반 상태 이석 체크·최초 로그인 시 설정 전체 조회 / 42·43차: 긴급해제 PHP·이석 잠금화면·보조 모니터 등) |
