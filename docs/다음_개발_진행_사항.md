...
(마지막 점검: 2026-02-16, 코드 기준 재검토 반영)
...
### 6. 프로세스 Kill 통제 + 관리자 OTP 승인 (Kill 통제·OTP 보고서)
...
### 8. 다중 디스플레이(보조모니터) 감지·처리 (다중 디스플레이 보고서)
...
### 9. Windows Defender/SmartScreen 경고 최소화 (SmartScreen 리포트)
...
### 10. 이석/절전 감지 구현 (이석·절전 감지 방안 리포트 + 절전모드-이석감지 연계 리포트)
...
### 11. 일반모드/임시연장/긴급사용 중 이석정보 서버 전송 (이석정보 서버 전송 리포트)
...
### 12. 시업/종업 화면 표시 로직 (시업·종업 화면 설계서)

- **목표**: 종업화면 / 시업화면 / 일반 PC 사용화면을 일관된 로직으로 결정
- **핵심 판단 포인트**
...
- **자율출근(시업시각 없음)**: 시업화면에서 PC-ON 버튼 활성화 → 출근처리 + 즉시 `USABLE`
- **상태머신 연계**
...
### 13. 고객사 설정 반영 (문구·이미지·로고·긴급해제·이석해제 비밀번호) (고객사 설정 리포트)
...
### 15. 트레이 에이전트 작동정보 조회 (트레이 작동정보 리포트)
