...
**해결**: 아래 중 하나를 수행한 뒤 앱을 다시 실행하면 **로그인 화면**이 나옵니다.
...
- **config.json에서 로그인 정보 제거**: 위 폴더 안의 `config.json`을 열어 `userServareaId`/`userStaffId` 항목을 삭제하고 `apiBaseUrl`만 남겨 둠

> **참고 (v0.1.7+)**: 신규 설치 시 번들 config에서 `apiBaseUrl`만 복사하므로 이 문제가 재발하지 않습니다.
